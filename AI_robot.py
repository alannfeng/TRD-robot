...
class MyClient(botpy.Client):
    ...
    async def ask_ai(user_id, ask):
      ...
      payload = {
        ...
        "options": {
          "imageIntention": {
            "needIntentionModel": False,
            "backendUpdateFlag": 2,

          }
        },
        "multimedia": [],
        "agentId": "RTrquvbPbd5L",
        "supportHint": 0,
        "version": "v2"
      }
      ...
      headers = {
        ...
        'Cookie': f'hy_source=web; hy_token={test_config["hy_token"]}; hy_user={test_config["hy_user"]}',
      ...
      for chunk in response.iter_content(chunk_size=8192):
          ...
          try:
              ...
              try:
                  ...
                  try:
                      decoded_chunk = chunk.decode('gbk')
                  except UnicodeDecodeError:
                      decoded_chunk = chunk.decode('utf-8', errors='replace')  # 替换无法解码的字符
              ...
              try:
                  # 只解析一次JSON
                  json_str = decoded_chunk[5:].strip()  # 去掉"data:"
                  if not json_str:
                      continue
                  ...
                  if data.get("type") == "text":
                      if "msg" in data:
                          ...
                          print(f"Warning: 'text' type message missing 'msg' field: {data}")
                  elif data.get("type") == "tool_calls":
                      # 处理工具调用类型的响应
                      print(f"Tool call received: {data.get('name', 'unnamed tool')}")
                      if "content" in data:
                          try:
                              content = json.loads(data["content"])
                              if "outputList" in content:
                                  for output in content["outputList"]:
                                      full_response += output.get("output", "")
                          except (json.JSONDecodeError, TypeError):
                              pass

              except json.JSONDecodeError as e:
                  print(f"JSON decode error on chunk: {e}\nChunk content: {decoded_chunk[:200]}...")
